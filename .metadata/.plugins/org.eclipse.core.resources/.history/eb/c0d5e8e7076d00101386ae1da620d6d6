/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32l4xx_hal.h"
#include "fonts.h"
#include "z_displ_ILI9XXX.h"
#include "z_displ_ILI9XXX_test.h"
#include "z_touch_XPT2046.h"
#include "z_touch_XPT2046_menu.h" // If you intend to use touch menu functions
#include "hx711.h"   // HX711 driver header
#include "hx711Config.h"
#include "PWMDriver.h"
#include "MuxDriver.h"
#include "HardwareTest.h"

extern CRC_HandleTypeDef hcrc;
extern SPI_HandleTypeDef hspi1;
extern DMA_HandleTypeDef hdma_spi1_tx;
extern TIM_HandleTypeDef htim2;
extern TIM_HandleTypeDef htim3;
extern TIM_HandleTypeDef htim4;
extern TIM_HandleTypeDef htim5;

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define DISP_LED_Pin GPIO_PIN_0
#define DISP_LED_GPIO_Port GPIOA
#define PUMP_PWM_Pin GPIO_PIN_1
#define PUMP_PWM_GPIO_Port GPIOA
#define DISPL_SCK_Pin GPIO_PIN_5
#define DISPL_SCK_GPIO_Port GPIOA
#define TOUCH_MISO_Pin GPIO_PIN_6
#define TOUCH_MISO_GPIO_Port GPIOA
#define DISPL_MOSI_Pin GPIO_PIN_7
#define DISPL_MOSI_GPIO_Port GPIOA
#define DISPL_CS_Pin GPIO_PIN_4
#define DISPL_CS_GPIO_Port GPIOC
#define TOUCH_CS_Pin GPIO_PIN_5
#define TOUCH_CS_GPIO_Port GPIOC
#define DISPL_DC_Pin GPIO_PIN_0
#define DISPL_DC_GPIO_Port GPIOB
#define DISPL_RST_Pin GPIO_PIN_1
#define DISPL_RST_GPIO_Port GPIOB
#define TOUCH_INT_Pin GPIO_PIN_10
#define TOUCH_INT_GPIO_Port GPIOB
#define TOUCH_INT_EXTI_IRQn EXTI15_10_IRQn
#define HX711_DOUT_Pin GPIO_PIN_13
#define HX711_DOUT_GPIO_Port GPIOB
#define HX711_SCK_Pin GPIO_PIN_14
#define HX711_SCK_GPIO_Port GPIOB
#define MUX_CTRL_C_Pin GPIO_PIN_6
#define MUX_CTRL_C_GPIO_Port GPIOC
#define MUX_CTRL_B_Pin GPIO_PIN_7
#define MUX_CTRL_B_GPIO_Port GPIOC
#define MUX_CTRL_A_Pin GPIO_PIN_8
#define MUX_CTRL_A_GPIO_Port GPIOC
#define VALVE_CTRL_Pin GPIO_PIN_8
#define VALVE_CTRL_GPIO_Port GPIOA
#define BUZZER_PWM_Pin GPIO_PIN_6
#define BUZZER_PWM_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
